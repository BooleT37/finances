...
- **Month convention**: Always use **1-based months (1-12)** everywhere — in Jotai atoms (`selectedMonthAtom`), API calls, DB fields, and all business logic. The only place to convert to 0-based is when directly calling dayjs APIs that require it (e.g. `today.month() + 1 === month` or `new Date(year, month - 1)`). Use `selectedMonthKeyAtom` (string `'YYYY-MM'`) only for display/storage; use `selectedMonthAtom` (1-based number) for all logic.
