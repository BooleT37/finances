...
- **`role` (`user`/`admin`) only gates the project-users page and the project-rename mutation** (`/settings/users` is admin-only via `adminMiddleware`; `/settings/project` is open to everyone but only admins can rename via `renameProject`) — every other feature is equally accessible to any project member regardless of role. See [Project Users](src/features/projectUsers/PROJECT_USERS.md) and [Project](src/features/project/PROJECT.md).
