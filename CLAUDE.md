...
- **Bootstrapping a new environment** (fresh `Project` + first admin `User` + `ProjectSetting`) is done via `scripts/bootstrap-project.ts`, never through the UI (sign-up is closed). `scripts/reset-password.ts` resets any user's password directly on any environment, for when no admin can sign in to use the in-app reset UI. Both prompt interactively for credentials/connection strings rather than taking them as arguments or hardcoding them — never hardcode a real secret or connection string in these files even though they're tracked in git. **One-off scripts don't belong in the repo** — name them `scripts/*.local.ts`, which is gitignored (the one-time pre-auth data migration was done this way).
