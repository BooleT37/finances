...
- [Project](src/features/project/PROJECT.md)
...
- **Every table must have RLS enabled.** Production is Supabase, which auto-grants `anon`/`authenticated` — the roles behind its public PostgREST API — *every* privilege on each new table in `public` (see `pg_default_acl`). Row-level security with no policies is the only thing making a table unreachable for them; the app's own role bypasses RLS, so enabling it costs nothing and no policies are needed. **Any migration that creates a table must end with `ALTER TABLE "<Name>" ENABLE ROW LEVEL SECURITY;`** — without it, that table (and `Session`/`Account` hold session tokens and password hashes) is world-readable and -writable via the project's anon key the moment it ships. `20260716120000_enable_row_level_security` backfilled this for every existing table.
- **Authoring a migration.** `prisma migrate dev --create-only` refuses to run in a non-interactive shell, so generate the DDL with `npx prisma migrate diff --from-config-datasource --to-schema prisma/schema.prisma --script`, save it as `prisma/migrations/<YYYYMMDDHHMMSS>_<name>/migration.sql`, and apply it with `npx prisma migrate deploy`. **Wrap the whole file in `BEGIN;` / `COMMIT;`** — Prisma sends the statements one at a time with no implicit transaction, so a failure halfway through leaves the database partially migrated *and* the migration marked failed, which then has to be untangled by hand. Afterwards run one e2e spec (`npx playwright test test/e2e/budgeting/budgeting.editing.spec.ts`): the Playwright harness applies every migration to a clean container, which is the only thing that proves the migration works from scratch rather than only against your local database.
- **`role` (`user`/`admin`) only gates the project-rename mutation and the project-users section** — both live on the same `/settings/project` page (open to everyone), composed at the route level from two different features. Only admins can rename via `renameProject`, and the users-management section (`ProjectUsersTable`) is only rendered for `session.role === 'admin'`, since `fetchProjectUsers` is itself `adminMiddleware`-gated. Every other feature is equally accessible to any project member regardless of role. See [Project Users](src/features/projectUsers/PROJECT_USERS.md) and [Project](src/features/project/PROJECT.md).
- **Bootstrapping a new environment** (fresh `Project` + first admin `User` + `ProjectSetting`) is done via `scripts/bootstrap-project.ts`, never through the UI (sign-up is closed). **Nobody can reset another user's password from the app** — a user can only change their own (current password required, via Better Auth's built-in `changePassword`). `scripts/reset-password.ts` resets any user's password directly on any environment, for when someone forgets theirs and can't sign in to change it themselves. Both scripts prompt interactively for credentials/connection strings rather than taking them as arguments or hardcoding them — never hardcode a real secret or connection string in these files even though they're tracked in git. **One-off scripts don't belong in the repo** — name them `scripts/*.local.ts`, which is gitignored (the one-time pre-auth data migration was done this way).

## Notes

- Run `npm run dev` to auto-generate routeTree.gen.ts when routes change
- **Dates**: Use [dayjs](https://day.js.org/) for all date handling — never native `Date`. The `datetimeCodec` in `src/shared/codecs.ts` decodes ISO strings to `dayjs.Dayjs` objects. The only exception is Prisma `where` clauses, which require native `Date`. As of Mantine v9, the date components (`DatePickerInput`, `MonthPickerInput`, `YearPickerInput`, `MonthPicker`, `YearPicker`, etc.) work with **string** values (`ISO_DATE_FORMAT`), not `Date` — their `value`/`onChange` and `getDayProps` deal in strings. Form state for date fields follows suit and is typed `string | null` (never `Date`/`dayjs`); convert to/from `dayjs` only at the API read/write boundary (e.g. `transactionToFormValues.ts`, the save atoms in `transactionSidebarMolecule.ts`/`subscriptionInlineEditMolecule.ts`). The shared `DatePickerWithTodayInput` wrapper is a thin pass-through around `DatePickerInput` (adds the today-highlight via `getDayProps`) — it performs no type conversion.
- **Date-format constants**: Never hardcode a dayjs format string. Always use the constants from `~/shared/constants`: `ISO_DATE_FORMAT` (`'YYYY-MM-DD'` — the machine/interchange format used for the backend API *and* Mantine date-component string values), `DATE_FORMAT` (`'DD.MM.YYYY'` — human display) and `MONTH_DATE_FORMAT` (`'MMMM YYYY'` — human display). If you need a format none of these covers, add a new named constant there rather than inlining a literal. Wherever a parameter, field, or state value is typed `string` and represents a date (e.g. a form's `date`/`firstDate`, a Mantine date-component value), assume `ISO_DATE_FORMAT` — no need to annotate this per-field with a comment.
...
- **Icon-only row actions**: Any row-action button that's icon-only (no visible label, e.g. an `ActionIcon` for edit/copy/delete in a table row) always requires a `Tooltip` wrapping it, even when the action seems self-explanatory from the icon alone — hovering must reveal what the button does. If the button can also be `disabled`, wrap it in a single `Tooltip` whose `label` switches between the normal action description and a reason-for-disabled message (see `RowActions.tsx` in `categories`/`transactions` for the pattern), rather than toggling the `Tooltip`'s own `disabled` prop — the user should always get a tooltip, whichever one applies.
...
- **Costed hover lists**: When rendering a hover popup with a list of costed things — transactions, subscriptions, forecast line items — use the shared `CostList` component (`~/shared/components/CostList`) inside a Mantine `HoverCard` — never a dark `Tooltip`, and don't hand-roll the rows. `CostList` is only the dropdown content (wrap it in `HoverCard` yourself); it takes `items` (`{ key, name, cost, extra?, secondary? }`), an optional `title`, and an optional `limit` (shows a "show more" link beyond that count; omit to show all). `extra` is a dimmed trailing detail the caller formats itself — a date (via `DATE_FORMAT`) for transactions and subscriptions, the formula and quantity for forecast line items, nothing when there's nothing to add. Use `secondary: true` to dim a row (e.g. an already-paid subscription) and fold any suffix like "(оплачено)" into `name`.
...
- **Grouping arrays**: Always prefer `Object.groupBy(items, keyFn)` over manually accumulating into a `Map`/object with a `for` loop. Node/TS support it (target/lib include ES2024) — reach for it first rather than hand-rolling the accumulator. Its result is a `Partial<Record<K, T[]>>`, so a value can be `undefined`; guard for that when iterating (e.g. `if (!group) continue;`).
