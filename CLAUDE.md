...
- **Mutations**: Prefer `mutate` with an `onSuccess` callback over `mutateAsync` + `await`. Only use `mutateAsync` when you need to do something with the result outside the callback (e.g. chaining two mutations).
