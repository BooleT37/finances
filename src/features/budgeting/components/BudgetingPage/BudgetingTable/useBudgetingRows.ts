...
import type { DueSubscription } from './fillFromSubscriptions';
import {
...
function useBuildCategoryRows(month: number, year: number) {
  ...
  const resolvedForecasts = useMemo(() => forecasts ?? [], [forecasts]);
  ...
  const toDueSubscriptions = useCallback(
    (subs: AvailableSubscription[]): DueSubscription[] =>
      subs.map((s) => ({
        subscriptionId: s.subscription.id,
        cost: s.subscription.cost.abs(),
        comment: t('breakdown.subscriptionComment', {
  ...
  return useCallback(
    (categories: Category[], ta: TransactionActuals): BudgetingRow[] => {
      const forecasts = resolvedForecasts;
      const subscriptions = resolvedSubscriptions;
      ...
      return categories.map((category) => {
        ...
        if (category.subcategories.length === 0) {
          ...
          const { average, monthCount } = ta.averages.getCategoryTotal(
            category.id,
          );
          ...
          return {
            ...
            subscriptions: {
              list: ownSubs,
              plans: buildCategoryFillPlans(
                category.id,
                {
                  lineItems: catForecast?.lineItems ?? [],
                  sum: categoryPlanSum.abs(),
                },
                toDueSubscriptions(ownSubs),
              ),
            },
            lineItems: catForecast?.lineItems ?? [],
                ...
                plans: buildSubcategoriesFillPlans(category.id, [
                  {
                    ...
                    due: toDueSubscriptions(subSubs),
                  },
                ]),
              },
              lineItems: subForecast?.lineItems ?? [],
              ...
              plans: buildSubcategoriesFillPlans(category.id, [
                {
                  ...
                  due: toDueSubscriptions(restSubs),
                },
              ]),
            },
            lineItems: restForecast?.lineItems ?? [],
            ...
            plans: buildSubcategoriesFillPlans(
              ...
              subRows.map((child) => ({
                ...
                current: {
                  ...
                  sum: child.planSum.abs(),
                },
                due: toDueSubscriptions(child.subscriptions.list),
              })),
            ),
          },
          lineItems: catForecast?.lineItems ?? [],
...
export function useBudgetingRows(
  ...
  const { sortAllCategoriesById, isSuccess: isCategoriesOrderLoaded } =
    useSortAllCategoriesById();
  const subscriptions = useAvailableSubscriptions();
  const buildCategoryRows = useBuildCategoryRows(month, year);
  ...
  >(() => {
    ...
    const expenseRows = buildCategoryRows(expenseCategories, ta);
    const incomeRows = buildCategoryRows(incomeCategories, ta);
    const savingsRows = buildCategoryRows(savingsCategories, ta);
